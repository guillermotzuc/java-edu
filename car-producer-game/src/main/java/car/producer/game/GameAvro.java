...
public class GameAvro extends JPanel{
	
	private KafkaProducer<String, Score> producer;
    ...
    public GameAvro(){
        ...
        /* Create a producer */
        this.producer = ProducerWithKey.newProducer2();
    }
    ...
    public void paint(Graphics g){
        ...
        try{
            obj.drawImage(getToolkit().getImage(getResourceBasepath() + "st_road.png"), 0, 0 ,this); //draw road on window
            if(cry >= -499 && crx >= -499) //if a road crossing has passed the window view
                obj.drawImage(getToolkit().getImage(getResourceBasepath() + "cross_road.png"),crx,cry,this); //draw another road crossing on window
            
            obj.drawImage(getToolkit().getImage(getResourceBasepath() + "car_self.png"),car_x,car_y,this);   //draw car on window
            
            if(isFinished){ //if collision occurs
                obj.drawImage(getToolkit().getImage(getResourceBasepath() + "boom.png"),car_x-30,car_y-30,this); //draw explosion image on window at collision to indicate the collision has occured
            }
            ...
            if((ly[i] >= car_y && ly[i] <= car_y+46) || (ly[i]+46 >= car_y && ly[i]+46 <= car_y+46)){   //if the cars collide vertically
                if(car_x+87 >= lx[i] && !(car_x >= lx[i]+87)){  //and if the cars collide horizontally
                    ...
                    try {
						...
						// TODO Auto-generated catch block
						e.printStackTrace();
					} //end game and print end message
                }
            }
        }
    }
    ...
    public static void main(String args[]){
        ...
        GameAvro game = new GameAvro(); //creating a new instance of a Game
        ...
        while(true){
            ...
            if(game.nOpponent < 4 && count % 200 == 0){ //if there is less than 4 cars and count timer reaches 200
                game.imageLoc[game.nOpponent] = getResourceBasepath() + "car_left_"+((int)((Math.random()*100)%3)+1)+".png"; //assign images to the opponent cars
    ...
    private static String getResourceBasepath() {
    	...
    	return "/Users/gtzuc/eclipse-workspace/2021/program/w1/car-producer-game/src/main/resources/images/";
    }
    
    //function that will display message after user has lost the game
    void finish()  throws ExecutionException, InterruptedException{
       ...
       GameUser user = RandomUserFactory.newUser();
       Score userScore = Score.newBuilder()
				        .setScore(score)
				        .setUsername(user.getName())
				        .setLocation(user.getLocation())
				        .build();
       
		String topic = "user_score";
		String key = user.getName();

		ProducerRecord<String, Score> record =
				new ProducerRecord<String, Score>(topic, key, userScore);
